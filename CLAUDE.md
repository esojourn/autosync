...
- **`autosync-watch.sh`** — The sync daemon. Runs continuously as a systemd service. On startup it does an initial sync of all folders to all configured hosts, then enters a loop: `inotifywait` blocks until a local file change or timeout (60s). Local changes trigger a debounced sync; timeouts trigger a periodic sync. Each sync calls `unison` per folder per host with `-auto -batch -prefer newer`. If a sync fails with a corrupt-archive signature (`loading archive`), it self-heals: one retry with `-ignorearchives` rebuilds both archive files, then declares OK/FAILED. Writes machine-readable status to `~/.local/share/autosync/status` (pipe-delimited: `HOST|FOLDER|STATUS|TIMESTAMP|TRIGGER`) and current state to `~/.local/share/autosync/state`.
